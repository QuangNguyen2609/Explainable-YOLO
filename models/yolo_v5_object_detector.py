...
class YOLOV5TorchObjectDetector(nn.Module):
    ...
    def forward(self, img):
        print("[DEBUG| yolov5_object_detector.py] cuda memory before model forward: ", torch.cuda.memory_allocated()/10**8)
        prediction, logits, _ = self.model(img, augment=False)
        channel = img.shape[0]
        del img
        print("[DEBUG| yolov5_object_detector.py] cuda memory after model forward: ", torch.cuda.memory_allocated()/10**8)
        prediction, logits = self.non_max_suppression(prediction, logits, self.confidence, self.iou_thresh,
                                                      classes=None,
                                                      agnostic=self.agnostic)
        
        self.boxes, self.class_names, self.classes, self.confidences = [[[] for _ in range(channel)] for _ in
                                                                        range(4)]
        ...
        for i, det in enumerate(prediction):  # detections per image
            if len(det):
                ...
                # del det, cls, conf, xyxy, bbox
        #del all variables in cuda
        torch.cuda.empty_cache()
        # del prediction, self.model
